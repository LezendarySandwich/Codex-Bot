...
async def parse_submissions_response(username: str, response: str, collection):
    '''
    ...
    for submission in submissions:
        if 'verdict' in submission and submission['verdict'] == 'OK':
            ...
            if not db.check_solved(handle=username, link=problem.url_get(), collection=collection):
                db.insert_users_db(handle=username, link=problem.url_get(), collection=collection)
                new_solved_problems.append(problem)
    return new_solved_problems


async def get_latest_submissions(username: str, collection):
    '''
    ...
    try :
        ...
        new_solved = await parse_submissions_response(username, response, collection)
        return new_solved
